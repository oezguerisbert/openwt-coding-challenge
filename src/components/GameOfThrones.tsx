import { lazy, ReactNode, Suspense, useContext, useEffect, useState } from "react";
...
const GameOfThrones = () => {
  ...
  const [selectedResource, setSelectedResource] = useState<Resource>(Resource.Books);
  ...
  return <div className="flex flex-col gap-4 w-full">
    <div className="flex flex-row gap-2">
      {resources && Object.entries(ResourcesList).map(([k, name]) => <button className={`${selectedResource === name ? "bg-blue-400" : "bg-slate-100"} flex px-3 py-2 justify-center items-center hover:bg-slate-300`} key={name} value={name} onClick={() => {
        setSelectedResource(name as Resource);
      }}>{name}</button>)}
      <div className="flex flex-grow" />
      <input className="flex w-1/3" onChange={(e) => { setSearch(e.currentTarget.value); }} placeholder="Search..." />
      <button className="flex p-2 py-3 bg-slate-600 text-white mx-auto w-auto px-8 font-bold" onClick={() => {
